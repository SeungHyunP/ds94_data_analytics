...
from Model.CRD_Soft import Soft_Triple, DistillKL, CRDLoss
from Train.CRD_Soft import *
...
# Option
def parse_args():
    parser = argparse.ArgumentParser(description='BaseLine + Soft + CRD Model')
...
if __name__ == '__main__':
    ...
    for i, hyper_parameter in enumerate(tqdm(hyperparameter_list, desc='Hyperparameter Search...')):
        ...
        # Train Teacher Model
        model = Soft_Triple([CFG['In_Hidden']] + CFG['In_hidden_list'], device)
        model.eval()
        ...
        # Student Model
        student_model = Soft_Triple([CFG['S_In_Hidden']] + CFG['In_hidden_list'], device).to(device)
        student_model.eval()
        ...
        feat_t, _ = teacher_model(X_t.to(device), y.float().to(device), is_feat=True)
        feat_s, _ = student_model(X_s.to(device), y.float().to(device), is_feat=True)
        ...
        hyper_parameters.save_val_test({'T-F1': T_F1, 'S-F1': S_F1, 'Final': best_score},
                                       sampling + 'Base_Soft_CRD', str_hyper_parameter_list,
                                       str_hyper_parameter_list[i])

        # Test
        test_save_path = './Result/Submit/' + sampling + 'Base_Soft_CRD'
