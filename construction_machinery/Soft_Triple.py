...
from Model.Soft_Triple import Soft_Triple
from Train.Soft_Triple import *
from torch.utils.data import DataLoader

# Option
def parse_args():
    parser = argparse.ArgumentParser(description='BaseLine + SoftTriple Loss Model')
...
if __name__ == '__main__':
    ...
    for i, hyper_parameter in enumerate(tqdm(hyperparameter_list, desc='Hyperparameter Search...')):
        ...
        # Student Model
        student_model = Soft_Triple([CFG['S_In_Hidden']] + CFG['In_hidden_list'], device)
        student_model.eval()
        ...
        hyper_parameters.save_val_test({'T-F1': T_F1, 'S-F1': S_F1, 'Final': best_score},
                                       sampling+'Base_SfotTriple',  str_hyper_parameter_list,
                                       str_hyper_parameter_list[i])

        # Test
        test_save_path = './Result/Submit/'+sampling+'Base_SfotTriple'
